...
#自己紹介 に投稿
  ...
  → Gemini API (gemini-3.5-flash) で「情報抽出 + 必要時のみ Google 検索 + お題生成」
    ...
    → 不合格なら理由を添えて1回だけ作り直す
  → 形式ごとの絵文字を付けた「お題」として #雑談 に投稿
    （二択形式のときは Discord の投票機能を使用）
  → 6時間後・24時間後に返信（スレッド内の発言を含む）・リアクション・投票を集計し、
    反応が良かったお題は次回以降の生成プロンプトに例として還元
```

Python + discord.py + google-genai。本体は約1,800行・5ファイル（＋プロンプト2ファイル）の小さな実装で、
主要ロジック（静穏判定・候補選択・形式の重み計算・計測時点の判定・文面の組み立て・過去ログ取り込みの判定）には
pytest のテスト132件を用意しています。
...
| TOPIC_FOOTER | -（なし） | お題の末尾に添える一言（例:「リアクションだけでも歓迎です」） |
