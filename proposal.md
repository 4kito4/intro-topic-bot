...
見出し（1行目）と末尾の一言は設定で変更でき、**毎回同じ体裁**で投稿されます。お題の形式によって見出しや装飾を変えることはしません。見出しの後ろには、その回のお題の題材を表す短い語（上の例では「記憶と自分」）を自動で添えます。Discord の通知やチャンネル一覧のプレビューは本文の先頭しか表示しないため、**開く前に何の話題かが伝わる**ようにするためのものです。

**見た目の設計判断**: この機能はあえて embed（枠付きカード）を使わず、プレーンテキストの固定フォーマットで投稿します。理由は4つあります。第一に、投稿するのは雑談チャンネルが静かなタイミングだけなので、お題は必ず「最新のメッセージ」として目に入り、embed の枠で目立たせる必要が構造的にありません。第二に、プレーンテキストなら**返信の引用に問いがそのまま乗る**ため、あとから流れを追う人にも何への返信か伝わります（embed は返信引用に本文が乗りません）。第三に、embed は「埋め込みを表示しない」というユーザー側の設定で消えてしまう可能性がありますが、通常のメッセージなら誰の環境でも同じように見えます。第四に、**通知とチャンネル一覧のプレビューに表示されるのはプレーン本文だけ**で、embed の中身はどちらにも乗りません。静かなタイミングにしか投稿しない以上、その場でチャンネルを開いている人はほとんどおらず、通知とプレビューが実質的に唯一の到達経路になります。見出しにテーマ語を添えているのはこのためで、装飾で目を引くより、雑談の流れに自然に混ざって実際に返信が付くことを優先した形です。
...
Python + discord.py + google-genai。本体は約2,000行・パッケージ5ファイル（＋プロンプト2ファイル）の小さな実装で、
主要ロジック（静穏判定・候補選択・形式の重み計算・計測時点の判定・文面の組み立て・過去ログ取り込みの判定・
既存 bot への組み込み）には pytest のテスト244件を用意しています。

## 4. 組み込み方法（2案）

### 案A: 既存 Bot へ統合（推奨）

機能一式を `intro_topic/` という1つのフォルダにまとめてあり、**そのフォルダを既存 Bot のリポジトリへコピーし、`await bot.load_extension("intro_topic")` を1行足すだけ**で載ります（既存 Bot が Python + discord.py の場合）。

- メリット: **追加のホスティングが要りません**。すでに動いている Bot のプロセスに相乗りするので、サーバーの用意・監視・課金がそのまま1つで済みます
- 既存 Bot への影響: イベントは追加式のリスナー（`Cog.listener`）として登録するため、既存の `on_message` などを奪いません。独自のプレフィックスコマンドも持たず、スラッシュコマンドの同期タイミングも既存 Bot の運用に任せます
- ロールバック: `load_extension` の1行を消して再起動するだけ（稼働中に `unload_extension` でその場撤収も可能）
- **運用者向けの統合手順書（`INTEGRATION.md`）を同梱**しています。前提条件・置き場所・環境変数・干渉しないことの確認・更新とロールバックまで、この1本で完了します

### 案B: 別 Bot として並走

既存 Bot のコードに一切触れず、この Bot を追加招待して別プロセスで動かします。既存 Bot が discord.py 製でない場合や、コードを触りたくない場合はこちらです。

- メリット: 既存 Bot への影響ゼロ、ロールバックは Bot をキックするだけ
- 必要なもの: ホスティング先1つ（常駐プロセス。Railway / Koyeb / GCP e2-micro 等の無料〜少額枠で可）
- **`Dockerfile` と導入手順書（`DEPLOY.md`）を同梱**しています。Docker が動く環境ならコマンド数個（`docker compose up -d`）で常駐化でき、Docker を使わない場合の systemd 手順・PaaS へのデプロイ手順・更新とロールバックの手順も同じ文書にまとめてあります

なお話題生成部（`intro_topic/topic_generator.py` と `intro_topic/prompts/`）は Discord に一切依存しない「テキスト入力 → 構造化出力」なので、既存 Bot が**他言語**の場合も、プロンプトと制御ロジック（下記閾値）の仕様をそのまま再実装できます。

## 5. 必要な権限・Intent

| 項目 | 値 |
|---|---|
| OAuth2 スコープ | `bot` / `applications.commands`（後者は運用者向けスラッシュコマンドの登録に必要）。案A（統合）では既存 Bot がすでに持っていれば追加は不要です |
...
管理者権限・メンバー情報・DM へのアクセスは不要です。案A（統合）の場合、必要な権限はほとんどの Bot がすでに持っている範囲なので、追加の権限付与なしで済むことが多いはずです。

## 6. 運用者向けの管理コマンド

Discord 上から `/topic` で操作できます。**一般メンバーにはコマンドの存在自体が見えません** — 一覧に表示されるのはサーバー管理権限を持つ人だけで、さらに実際に実行できるのは設定で指定した運用者（ユーザー本人、または運用ロールを持つメンバー）だけです。応答も本人にしか見えない形式（ephemeral）なので、チャンネルには何も残りません。

**運営が複数人の場合**は、ユーザー ID の代わりに（または併用して）「Bot 運用」のようなロールの ID を設定できます（`OWNER_ROLE_ID`）。そのロールを付け外しするだけで担当者を入れ替えられ、設定の変更や再起動は要りません。なお「実行できるか」と「コマンド一覧に出るか」は別の仕組みで、一覧に出るのはサーバー管理権限を持つ人だけという既定は変わりません。運用ロールにサーバー管理権限が無い場合は、サーバー設定の「連携サービス」から `/topic` の表示先を上書きしていただく形になります。
...
| OWNER_USER_ID | -（無効） | `/topic` 管理コマンドを実行できる運用者のユーザー ID |
| OWNER_ROLE_ID | -（無効） | `/topic` を実行できる運用ロールの ID。上記との OR 判定（両方未設定だと誰も実行できません） |
...
- 案A（統合）は既存 Bot が **Python + discord.py 2.4 以上**であることが前提です（Python は 3.10 以上）。それ以外のスタックの場合は案B（並走）か、仕様に基づく再実装になります
- 案B（並走）を選ぶ場合は、常駐ホスティング先をご用意いただく必要があります（動かす場所そのものは同梱していません）。コンテナ対応は済んでおり、`Dockerfile` と導入手順書（`DEPLOY.md`）を同梱しているため、Docker が動く環境・PaaS・Linux VPS（systemd）のいずれでも手順どおりに載せられます
- Railway / Koyeb などファイルシステムが揮発するホスティングでは、再起動で状態（処理済み ID・再利用プール）が失われます。永続ボリュームをマウントし、`STATE_PATH` をその上のパスに向けてください（同梱の Docker イメージは `/data/state.json` が既定です）
