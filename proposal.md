...
- 常駐ホスティング先はご用意いただく必要があります（動かす場所そのものは同梱していません）。コンテナ対応は済んでおり、`Dockerfile` と導入手順書（`DEPLOY.md`）を同梱しているため、Docker が動く環境・PaaS・Linux VPS（systemd）のいずれでも手順どおりに載せられます
- Railway / Koyeb などファイルシステムが揮発するホスティングでは、再起動で状態（処理済み ID・再利用プール）が失われます。永続ボリュームを `/data` にマウントし、`STATE_PATH` をその上のパス（`/data/state.json`）に向けてください（同梱の Docker イメージは既定でこの構成です）
